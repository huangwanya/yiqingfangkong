<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.xinxifengmian" label="信息封面" prop="xinxifengmian">
          <file-upload
          tip="点击上传信息封面"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.xinxifengmian?ruleForm.xinxifengmian:''"
          @change="xinxifengmianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.xinxifengmian" label="信息封面" prop="xinxifengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.xinxifengmian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="疫情地址" prop="yiqingdizhi">
          <el-input v-model="ruleForm.yiqingdizhi" 
              placeholder="疫情地址" clearable  :readonly="ro.yiqingdizhi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="疫情地址" prop="yiqingdizhi">
              <el-input v-model="ruleForm.yiqingdizhi" 
                placeholder="疫情地址" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="活动踪迹" prop="huodongzongji">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="活动踪迹"
                  v-model="ruleForm.huodongzongji" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.huodongzongji" label="活动踪迹" prop="huodongzongji">
                    <span>{{ruleForm.huodongzongji}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	faburiqi : false,
      ...
      rules: {
          ...
          yiqingdizhi: [
                { required: true, message: '疫情地址不能为空', trigger: 'blur' },
          ],
          huodongzongji: [
          ],
          xinxineirong: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xinximingcheng = true;
          ...
          if(o=='xinxifengmian'){
            this.ruleForm.xinxifengmian = obj[o];
	    this.ro.xinxifengmian = true;
            continue;
          }
          if(o=='yiqingdizhi'){
            this.ruleForm.yiqingdizhi = obj[o];
	    this.ro.yiqingdizhi = true;
            continue;
          }
          if(o=='huodongzongji'){
            this.ruleForm.huodongzongji = obj[o];
	    this.ro.huodongzongji = true;
            continue;
          }
          if(o=='xinxineirong'){
	...
	this.ruleForm.xinxineirong = this.ruleForm.xinxineirong.replace(reg,'../../../ssmn87ov/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.xinxifengmian!=null) {
		this.ruleForm.xinxifengmian = this.ruleForm.xinxifengmian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `fangyixinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.fangyixinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    xinxifengmianUploadChange(fileUrls) {
	this.ruleForm.xinxifengmian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
